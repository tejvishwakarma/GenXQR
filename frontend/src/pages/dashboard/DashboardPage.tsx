...
function DownloadMenu({ qr }: { qr: QRCode }) {
  ...
  const handleDownload = useCallback(async (ext: DownloadExt) => {
    ...
    try {
      ...
      if (ext === "svg") {
        // Generate SVG at a clean base size, then post-process to make it
        // truly viewBox-based (no fixed width/height) for infinite scalability.
        ...
        if (blob) {
          // getRawData types as Blob | Buffer — handle both
          const raw = blob instanceof Blob ? await blob.text() : (blob as Buffer).toString("utf-8")
          // Strip fixed width/height from root <svg>, preserve/inject viewBox
          const cleaned = raw.replace(/^(<svg)([^>]*)(>)/, (_match: string, open: string, attrs: string, close: string) => {
            ...
            return `${open}${strippedAttrs}${viewBoxAttr}${close}`
          })
