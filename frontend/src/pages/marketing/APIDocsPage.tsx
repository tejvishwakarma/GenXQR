  ...
  return (
    <div className="mb-6">
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-zinc-800 bg-zinc-900/60">
              <th className="text-left px-4 py-2.5 text-zinc-500 font-medium w-36">Name</th>
              <th className="text-left px-4 py-2.5 text-zinc-500 font-medium w-24">Type</th>
              <th className="text-left px-4 py-2.5 text-zinc-500 font-medium">Description</th>
            </tr>
          </thead>
          <tbody>
            {fields.map((f, i) => (
              <tr key={f.name} className={cn("border-b border-zinc-800/50 last:border-0", i % 2 === 0 ? "bg-transparent" : "bg-zinc-900/20")}>
                <td className="px-4 py-2.5 font-mono text-violet-300 align-top">
                  {f.name}
                  {f.required && <span className="ml-1 text-red-400">*</span>}
                </td>
                <td className="px-4 py-2.5 font-mono text-zinc-500 align-top">{f.type}</td>
                <td className="px-4 py-2.5 text-zinc-400 align-top leading-relaxed">{f.desc}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {fields.some((f) => f.required) && (
        <p className="text-zinc-600 text-xs mt-1.5"><span className="text-red-400">*</span> Required</p>
      )}
    </div>
  )
}
      ...
      {LANGS.map((l) => (
        <button
          ...
          className={cn(
            ...
            lang === l.id ? "bg-zinc-700 text-white" : "text-zinc-500 hover:text-zinc-300"
          )}
        >
          {l.label}
        </button>
...
function EndpointSection({ ep, lang }: { ep: Endpoint; lang: Lang }) {
  return (
    <section id={ep.id} className="scroll-mt-20 mb-16">
      {/* Header */}
      <div className="flex items-center gap-3 mb-4">
        <MethodBadge method={ep.method} />
        <code className="text-zinc-200 text-sm font-mono bg-zinc-900 border border-zinc-800 px-3 py-1 rounded-lg">
          {ep.path}
        </code>
      </div>
      <h3 className="text-white font-bold text-xl mb-2">{ep.summary}</h3>
      <p className="text-zinc-400 text-sm leading-relaxed mb-6">{ep.description}</p>
      ...
      <div className="grid lg:grid-cols-[1fr_1fr] gap-6">
        <div>
          ...
          <div>
            <h4 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-3">Status Codes</h4>
            <div className="space-y-2">
              {ep.codes.map((c) => (
                <div key={c.code} className="flex items-center gap-3 text-xs">
                  <span className={cn(
                    "font-mono font-bold w-10 shrink-0",
                    c.code.startsWith("2") ? "text-emerald-400"
                      : c.code.startsWith("4") ? "text-amber-400"
                      : "text-red-400"
                  )}>{c.code}</span>
...
function IntroSection() {
  return (
    <section id="introduction" className="scroll-mt-20 mb-16">
      <h2 className="text-white font-bold text-2xl mb-4">Introduction</h2>
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-6">
        {[
          ...
          <div key={c.title} className="border border-zinc-800 rounded-xl p-4 bg-zinc-900/30">
            <div className="text-violet-400 mb-2">{c.icon}</div>
            <p className="text-white text-xs font-semibold mb-1">{c.title}</p>
...
function AuthSection() {
  return (
    <section id="authentication" className="scroll-mt-20 mb-16">
      <h2 className="text-white font-bold text-2xl mb-4">Authentication</h2>
      <p className="text-zinc-400 text-sm leading-relaxed mb-4">
        All API requests require an API key passed in the <code className="text-violet-300 bg-violet-500/10 px-1 rounded">Authorization</code> header using the Bearer scheme. Generate a key from{" "}
        <Link to="/app/api-keys" className="text-violet-400 hover:text-violet-300 underline underline-offset-2">API Keys</Link> in your dashboard.
      </p>
...
curl https://genxqr.streamsnatcher.com/v1/qr \\
  -H "Authorization: Bearer nxqr_••••••••••••••••••••••••••••••••"`} />
      <div className="mt-4 p-4 border border-amber-500/20 bg-amber-500/5 rounded-xl">
        <div className="flex items-start gap-3">
          <AlertTriangle size={15} className="text-amber-400 mt-0.5 shrink-0" />
          <div>
            <p className="text-amber-300 text-xs font-semibold mb-1">Keep your key secret</p>
            <p className="text-zinc-400 text-xs leading-relaxed">
              API keys carry full account access. Store them in environment variables (<code className="text-amber-300">GenXQR_API_KEY</code>), never in client-side code or source files.
...
function RateLimitsSection() {
  return (
    <section id="rate-limits" className="scroll-mt-20 mb-16">
      <h2 className="text-white font-bold text-2xl mb-4">Rate Limits</h2>
      <p className="text-zinc-400 text-sm leading-relaxed mb-4">
        Rate limits are applied per API key. When you exceed the limit, the API responds with <code className="text-amber-300 bg-amber-500/10 px-1 rounded">HTTP 429</code> and a <code className="text-zinc-300 bg-zinc-800 px-1 rounded">Retry-After</code> header telling you how many seconds to wait.
      </p>
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {[
              ...
              <tr key={label} className={cn("border-b border-zinc-800/50 last:border-0", i % 2 === 1 ? "bg-zinc-900/20" : "")}>
                <td className="px-4 py-2.5 text-zinc-300">{label}</td>
                <td className="px-4 py-2.5 font-mono text-violet-300">{val}</td>
...
function ErrorsSection() {
  return (
    <section id="errors" className="scroll-mt-20 mb-16">
      <h2 className="text-white font-bold text-2xl mb-4">Error Codes</h2>
      <p className="text-zinc-400 text-sm leading-relaxed mb-4">
        Errors always return a JSON body with <code className="text-violet-300 bg-violet-500/10 px-1 rounded">success: false</code> and a human-readable <code className="text-violet-300 bg-violet-500/10 px-1 rounded">error</code> field. Validation errors additionally include an <code className="text-violet-300 bg-violet-500/10 px-1 rounded">errors</code> array.
      </p>
    ...
    { "field": "content.data.url", "message": "URL is required" }
      ...
      <div className="mt-4 border border-zinc-800 rounded-xl overflow-hidden">
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-zinc-800 bg-zinc-900/60">
              <th className="text-left px-4 py-2.5 text-zinc-500 font-medium w-14">Code</th>
              <th className="text-left px-4 py-2.5 text-zinc-500 font-medium w-44">Title</th>
              <th className="text-left px-4 py-2.5 text-zinc-500 font-medium">Description</th>
            </tr>
          </thead>
          <tbody>
            {ERROR_CODES.map((e, i) => (
              <tr key={e.code} className={cn("border-b border-zinc-800/50 last:border-0", i % 2 === 1 ? "bg-zinc-900/20" : "")}>
                <td className="px-4 py-3 font-mono font-bold text-amber-400 align-top">{e.code}</td>
                <td className="px-4 py-3 text-zinc-300 align-top">{e.title}</td>
                <td className="px-4 py-3 text-zinc-500 leading-relaxed align-top">{e.desc}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function PaginationSection() {
  return (
    <section id="pagination" className="scroll-mt-20 mb-16">
      <h2 className="text-white font-bold text-2xl mb-4">Pagination</h2>
      <p className="text-zinc-400 text-sm leading-relaxed mb-4">
        All list endpoints use cursor-free page-based pagination. Pass <code className="text-violet-300 bg-violet-500/10 px-1 rounded">page</code> and <code className="text-violet-300 bg-violet-500/10 px-1 rounded">limit</code> as query parameters. Every list response includes a <code className="text-violet-300 bg-violet-500/10 px-1 rounded">meta</code> object.
      </p>
...
function WebhooksSection() {
  return (
    <>
      <section id="webhooks-overview" className="scroll-mt-20 mb-16">
        <h2 className="text-white font-bold text-2xl mb-4">Webhooks — Overview</h2>
        <p className="text-zinc-400 text-sm leading-relaxed mb-4">
        ...
        <p className="text-zinc-400 text-sm leading-relaxed mb-4">
          ...
          <Link to="/app/webhooks" className="text-violet-400 hover:text-violet-300 underline underline-offset-2">Webhooks</Link>{" "}
          page in your dashboard. Each webhook has its own HMAC secret used to verify requests.
        </p>
        <div className="border border-zinc-800 rounded-xl p-4 bg-zinc-900/30 font-mono text-xs space-y-1.5 mb-4">
      ...
      <section id="webhooks-events" className="scroll-mt-20 mb-16">
        <h2 className="text-white font-bold text-2xl mb-4">Events & Payloads</h2>
        <p className="text-zinc-400 text-sm leading-relaxed mb-6">
          Subscribe to any combination of the events below when creating a webhook.
        </p>
        <div className="space-y-5">
          {WEBHOOK_EVENTS.map((ev) => (
            <div key={ev.event} className="border border-zinc-800 rounded-xl overflow-hidden">
              <div className="flex items-center gap-3 px-4 py-3 bg-zinc-900/50 border-b border-zinc-800">
                <code className="text-violet-300 text-sm font-mono font-semibold">{ev.event}</code>
              </div>
              <div className="p-4">
                <p className="text-zinc-400 text-xs leading-relaxed mb-3">{ev.desc}</p>
      ...
      <section id="webhooks-verify" className="scroll-mt-20 mb-16">
        <h2 className="text-white font-bold text-2xl mb-4">Signature Verification</h2>
        <p className="text-zinc-400 text-sm leading-relaxed mb-4">
          Every webhook request includes an <code className="text-violet-300 bg-violet-500/10 px-1 rounded">X-GenXQR-Signature</code> header containing <code className="text-violet-300 bg-violet-500/10 px-1 rounded">sha256=&lt;HMAC&gt;</code>. The HMAC is computed from the raw request body using your webhook secret. Always verify this before processing the payload.
        </p>
        <div className="p-4 border border-amber-500/20 bg-amber-500/5 rounded-xl mb-5">
          <div className="flex items-start gap-3">
            <AlertTriangle size={15} className="text-amber-400 mt-0.5 shrink-0" />
            <p className="text-zinc-400 text-xs leading-relaxed">
              Use <strong className="text-amber-300">timing-safe comparison</strong> when checking the signature.
              Standard string equality (<code className="text-red-400">===</code>) is vulnerable to timing attacks.
              Use <code className="text-amber-300">crypto.timingSafeEqual</code> in Node.js or <code className="text-amber-300">hmac.compare_digest</code> in Python.
            </p>
          </div>
        </div>
        <div className="space-y-4">
              ...
              <li key={item.id}>
                ...
                >
                  {ENDPOINT_ICONS[item.id] && (
                    <span className="text-zinc-600">{ENDPOINT_ICONS[item.id]}</span>
                  )}
                  {item.label}
                </button>
              </li>
...
export default function APIDocsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white">
      ...
      {/* ── Top bar ── */}
      <div className="sticky top-0 z-40 border-b border-zinc-800 bg-[#09090b]/95 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-zinc-300 capitalize">{activeSection.replace("ep-", "").replace("-", " ")}</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <Link
              to="/app/api-keys"
              className="hidden sm:inline-flex items-center gap-2 px-4 py-1.5 rounded-lg bg-violet-600 hover:bg-violet-500 text-white text-xs font-semibold transition-colors"
            >
              <Key size={13} />
              Get API Key
            </Link>
      ...
      {mobileSidebarOpen && (
        <div className="fixed inset-0 z-30 lg:hidden">
          ...
          <div className="absolute left-0 top-14 bottom-0 w-64 bg-[#0f0f12] border-r border-zinc-800 overflow-y-auto p-5">
      ...
      <div className="max-w-7xl mx-auto flex">
        ...
        <aside className="hidden lg:block w-60 shrink-0 sticky top-14 self-start h-[calc(100vh-3.5rem)] overflow-y-auto border-r border-zinc-800/50 py-8 pr-4 pl-4">
          <Sidebar activeSection={activeSection} onNav={handleNav} />
        </aside>
        ...
        <main ref={contentRef} className="flex-1 min-w-0 px-6 lg:px-10 py-10 max-w-none">
          ...
          {/* Hero */}
          <div className="mb-14 pb-10 border-b border-zinc-800">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Link
                to="/app/webhooks"
                className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-zinc-700 hover:border-zinc-600 text-zinc-300 hover:text-white text-sm font-medium transition-colors"
              >
                <Radio size={14} />
                Set up webhooks
              </Link>
          ...
          <div className="mb-8">
            <h2 className="text-white font-bold text-2xl mb-1">QR Codes</h2>
            <p className="text-zinc-500 text-sm mb-8">Create, read, update, and delete QR codes in your account.</p>
          </div>
          ...
          <div className="mb-8">
            <h2 className="text-white font-bold text-2xl mb-1">Analytics</h2>
            <p className="text-zinc-500 text-sm mb-8">Retrieve scan analytics for your QR codes.</p>
          </div>
          ...
          <section className="scroll-mt-20 mb-16">
            <h2 className="text-white font-bold text-2xl mb-4">Content Schemas</h2>
            <p className="text-zinc-400 text-sm leading-relaxed mb-6">
              The <code className="text-violet-300 bg-violet-500/10 px-1 rounded">content</code> field structure depends on the QR type. Below are examples for the most common types.
            </p>
            <div className="grid sm:grid-cols-2 gap-4">
              {[
                ...
                <div key={item.type} className="border border-zinc-800 rounded-xl overflow-hidden">
          ...
          {/* CTA */}
          <div className="border border-zinc-800 rounded-2xl p-8 bg-zinc-900/30 text-center">
            <h3 className="text-white font-bold text-xl mb-2">Ready to build?</h3>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <Link
                to="/signup"
                className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl border border-zinc-700 hover:border-zinc-600 text-zinc-300 hover:text-white text-sm font-medium transition-colors"
              >
                Create free account
          ...
          <p className="text-zinc-700 text-xs text-center mt-10">
            Questions? <Link to="/contact" className="text-zinc-500 hover:text-zinc-400 underline underline-offset-2">Contact support</Link>
          </p>
        </main>
      </div>
    </div>
